...
#define MAXRECT 1000000 //1 millón
#define MAXTHREAD 10
#define BASE 0.0 //a
#define LIMIT 1.0 //b

double length = LIMIT - BASE; // La longitud en el eje 'x' // [a,b]
double numRectxThread = (double)(MAXRECT/MAXTHREAD); // Cuantos hilos/rectangulo -> 100000 cien mil
...
void * calcular(void *arg) {
	...
	//    variable 'loslimit'
	//    --> y en cual termina, para esto crearé dos variables
	double izq_loslimit = (*id+1)*(numRectxThread)-(numRectxThread);
	double der_loslimit = (*id+1)*(numRectxThread)-1;
	if((*id+1)==MAXTHREAD) der_loslimit++; //Se suma un rectangulo mas por ser el ultimo intervalo
	...
	while (!(izq_loslimit==der_loslimit)){
		partialSums[*id]+=function(der_loslimit) * base_length;
		der_loslimit--;
	}
	
	printf("[Interval %d] sum: %lf\n",*id+1, partialSums[*id]);
	pthread_exit(NULL);
}
